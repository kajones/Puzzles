...
";
        ...
        public void FixtureSetUp()
        {
            ...
            grid.ConfirmPossibilities(2, 4, new[] { 3, 6 });
            ...
            grid.ConfirmPossibilities(6, 4, new[] { 1, 4, 6 });
            ...
            grid.ConfirmPossibilities(7, 3, new[] { 1, 7 });
            grid.ConfirmPossibilities(7, 4, new[] { 1, 4, 6 });
        ...
        private static Grid GetGridAfterSomeSolving()
        {
            var grid = GridBuilder.BuildFromDefinition(exampleGrid);
            grid.Tidy();

            // This is the state of grid 7 after some general solving and then explicitly...
            grid.Solve(new DigitRestrictedToColumnInBoxSolver());
            grid.Solve(new DigitRestrictedToColumnsInBoxesSolver());
            return grid;
        }
        ...
        [Test]
        public void WhenThereIsANakedPairInARow_ThenThoseDigitsAreRemovedFromTheOtherSquaresInTheRow()
        {
            var grid = GetGridAfterSomeSolving();

            Console.WriteLine(grid.Summary());

            grid.Solve(new NakedPairSolver());

            grid.ConfirmPossibilities(7,0, new [] { 1,7}, "First pair intact");
            grid.ConfirmPossibilities(7,1,new [] { 6,9}, "Removed 7");
            grid.ConfirmPossibilities(7,2,new [] { 6,9}, "Removed 1 and 7");
            grid.ConfirmPossibilities(7,3,new [] { 1,7}, "Second pair intact");
            grid.ConfirmPossibilities(7,4,new [] { 4,6}, "Removed 1");
            grid.ConfirmPossibilities(7,5,new [] { 2,3,6}, "Removed 7");
            grid.ConfirmPossibilities(7,6,new [] { 3,4,6,8,9});
            grid.ConfirmPossibilities(7,7,new [] { 2,3,4,6,8});
        }

        [Test, Explicit]
        public void WhenThereIsANakedPairInAColumn_ThenThoseDigitsAreRemovedFromTheOtherSquares()
        {
            Assert.Fail("To implement");
        }

        [Test, Explicit]
        public void WhenThereIsANakedPairInABox_ThenThoseDigitsAreRemovedFromTheOtherSquares()
        {
            Assert.Fail("To implement");
        }
    }
